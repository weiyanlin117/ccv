...
// Removed: #include "../utils.cuh" (PyTorch dependencies)
#include <cuda_fp16.h>
#include <cuda_pipeline_primitives.h>
// Removed: #include <torch/extension.h> (PyTorch dependencies)

#include "../cp_async.cuh"
#include "../mma.cuh"
#include "../permuted_smem.cuh"
#include "../math.cuh"
// Removed: #include "../dispatch_utils.h" (PyTorch dependencies)
...
// Only the kernel template function - no PyTorch wrapper functions
...
// Explicit template instantiations for F8 kernels used in CCV

// Basic F8 kernel for head_dim=128, per-block quantization
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 32, 64, 128, DataType::kInt8, QuantGranularity::kPerBlock, QuantGranularity::kPerBlock, float, false, half, ComputeUnit::kTensorCore, MaskMode::kNone, false, false, false, false>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
    const uint32_t stride_bz_k, const uint32_t stride_seq_k, const uint32_t stride_h_k,
...
// Basic F8 kernel for head_dim=64, per-block quantization
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 32, 64, 64, DataType::kInt8, QuantGranularity::kPerBlock, QuantGranularity::kPerBlock, float, false, half, ComputeUnit::kTensorCore, MaskMode::kNone, false, false, false, false>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
// Per-warp Q, per-block K variants
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 32, 64, 128, DataType::kInt8, QuantGranularity::kPerWarp, QuantGranularity::kPerBlock, float, false, half, ComputeUnit::kTensorCore, MaskMode::kNone, false, false, false, false>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 32, 64, 64, DataType::kInt8, QuantGranularity::kPerWarp, QuantGranularity::kPerBlock, float, false, half, ComputeUnit::kTensorCore, MaskMode::kNone, false, false, false, false>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
// Per-warp Q, per-warp K variants
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 32, 64, 128, DataType::kInt8, QuantGranularity::kPerWarp, QuantGranularity::kPerWarp, float, false, half, ComputeUnit::kTensorCore, MaskMode::kNone, false, false, false, false>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 32, 64, 64, DataType::kInt8, QuantGranularity::kPerWarp, QuantGranularity::kPerWarp, float, false, half, ComputeUnit::kTensorCore, MaskMode::kNone, false, false, false, false>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
// Causal variants
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 32, 64, 128, DataType::kInt8, QuantGranularity::kPerWarp, QuantGranularity::kPerBlock, float, false, half, ComputeUnit::kTensorCore, MaskMode::kCausal, false, false, false, false>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 32, 64, 64, DataType::kInt8, QuantGranularity::kPerWarp, QuantGranularity::kPerBlock, float, false, half, ComputeUnit::kTensorCore, MaskMode::kCausal, false, false, false, false>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
// Instruction buffer variants (use_inst_buffer=true)
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 32, 64, 128, DataType::kInt8, QuantGranularity::kPerWarp, QuantGranularity::kPerWarp, float, true, half, ComputeUnit::kTensorCore, MaskMode::kNone, false, false, false, false>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 32, 64, 64, DataType::kInt8, QuantGranularity::kPerWarp, QuantGranularity::kPerWarp, float, true, half, ComputeUnit::kTensorCore, MaskMode::kNone, false, false, false, false>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
// Instruction buffer with FP16 accumulation (use_pv_fp16_accu=true)
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 32, 64, 128, DataType::kInt8, QuantGranularity::kPerWarp, QuantGranularity::kPerWarp, float, true, half, ComputeUnit::kTensorCore, MaskMode::kNone, false, false, false, true>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 32, 64, 64, DataType::kInt8, QuantGranularity::kPerWarp, QuantGranularity::kPerWarp, float, true, half, ComputeUnit::kTensorCore, MaskMode::kNone, false, false, false, true>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
// Per-thread quantization variants
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 16, 64, 128, DataType::kInt8, QuantGranularity::kPerThread, QuantGranularity::kPerThread, float, true, half, ComputeUnit::kTensorCore, MaskMode::kNone, false, false, false, false>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 32, 64, 64, DataType::kInt8, QuantGranularity::kPerThread, QuantGranularity::kPerThread, float, true, half, ComputeUnit::kTensorCore, MaskMode::kNone, false, false, false, false>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
// Missing template instantiations needed by qk_int8_sv_f8_accum_f16_fuse_v_scale_attn_inst_buf_direct
// These match the exact template parameters being called in the wrapper function

// HEAD_DIM=128, WARP_Q=16, QuantGranularity::kPerThread (3), fuse_v_scale=true, use_pv_fp16_accu=true, Non-causal
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 16, 64, 128, DataType::kInt8, QuantGranularity::kPerThread, QuantGranularity::kPerThread, float, true, half, ComputeUnit::kCudaCore, MaskMode::kNone, false, true, false, true>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
// HEAD_DIM=128, WARP_Q=16, QuantGranularity::kPerThread (3), fuse_v_scale=true, use_pv_fp16_accu=true, Causal
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 16, 64, 128, DataType::kInt8, QuantGranularity::kPerThread, QuantGranularity::kPerThread, float, true, half, ComputeUnit::kCudaCore, MaskMode::kCausal, false, true, false, true>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
// HEAD_DIM=64, WARP_Q=32, QuantGranularity::kPerThread (3), fuse_v_scale=true, use_pv_fp16_accu=true, Non-causal
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 32, 64, 64, DataType::kInt8, QuantGranularity::kPerThread, QuantGranularity::kPerThread, float, true, half, ComputeUnit::kCudaCore, MaskMode::kNone, false, true, false, true>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
// HEAD_DIM=64, WARP_Q=32, QuantGranularity::kPerThread (3), fuse_v_scale=true, use_pv_fp16_accu=true, Causal
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 32, 64, 64, DataType::kInt8, QuantGranularity::kPerThread, QuantGranularity::kPerThread, float, true, half, ComputeUnit::kCudaCore, MaskMode::kCausal, false, true, false, true>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
// HEAD_DIM=128, WARP_Q=16, QuantGranularity::kPerWarp (2), fuse_v_scale=true, use_pv_fp16_accu=true, Non-causal
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 16, 64, 128, DataType::kInt8, QuantGranularity::kPerWarp, QuantGranularity::kPerWarp, float, true, half, ComputeUnit::kCudaCore, MaskMode::kNone, false, true, false, true>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
// HEAD_DIM=128, WARP_Q=16, QuantGranularity::kPerWarp (2), fuse_v_scale=true, use_pv_fp16_accu=true, Causal
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 16, 64, 128, DataType::kInt8, QuantGranularity::kPerWarp, QuantGranularity::kPerWarp, float, true, half, ComputeUnit::kCudaCore, MaskMode::kCausal, false, true, false, true>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
// HEAD_DIM=64, WARP_Q=32, QuantGranularity::kPerWarp (2), fuse_v_scale=true, use_pv_fp16_accu=true, Non-causal
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 32, 64, 64, DataType::kInt8, QuantGranularity::kPerWarp, QuantGranularity::kPerWarp, float, true, half, ComputeUnit::kCudaCore, MaskMode::kNone, false, true, false, true>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
// HEAD_DIM=64, WARP_Q=32, QuantGranularity::kPerWarp (2), fuse_v_scale=true, use_pv_fp16_accu=false, Non-causal
    template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 32, 64, 64, DataType::kInt8, QuantGranularity::kPerWarp, QuantGranularity::kPerWarp, float, true, half, ComputeUnit::kCudaCore, MaskMode::kNone, false, true, false, false>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
// HEAD_DIM=64, WARP_Q=32, QuantGranularity::kPerWarp (2), fuse_v_scale=true, use_pv_fp16_accu=true, Causal
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 32, 64, 64, DataType::kInt8, QuantGranularity::kPerWarp, QuantGranularity::kPerWarp, float, true, half, ComputeUnit::kCudaCore, MaskMode::kCausal, false, true, false, true>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
// HEAD_DIM=64, WARP_Q=32, QuantGranularity::kPerWarp (2), fuse_v_scale=true, use_pv_fp16_accu=false, Causal
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 32, 64, 64, DataType::kInt8, QuantGranularity::kPerWarp, QuantGranularity::kPerWarp, float, true, half, ComputeUnit::kCudaCore, MaskMode::kCausal, false, true, false, false>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
// HEAD_DIM=128, WARP_Q=16, QuantGranularity::kPerThread (3), fuse_v_scale=true, use_inst_buffer=true, Non-causal
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 16, 64, 128, DataType::kInt8, QuantGranularity::kPerThread, QuantGranularity::kPerThread, float, true, half, ComputeUnit::kTensorCore, MaskMode::kNone, false, true, false, false>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
// HEAD_DIM=64, WARP_Q=32, QuantGranularity::kPerThread (3), fuse_v_scale=true, use_inst_buffer=true, Non-causal
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 32, 64, 64, DataType::kInt8, QuantGranularity::kPerThread, QuantGranularity::kPerThread, float, true, half, ComputeUnit::kTensorCore, MaskMode::kNone, false, true, false, false>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
// HEAD_DIM=128, WARP_Q=16, QuantGranularity::kPerThread (3), fuse_v_scale=true, use_inst_buffer=true, Causal
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 16, 64, 128, DataType::kInt8, QuantGranularity::kPerThread, QuantGranularity::kPerThread, float, true, half, ComputeUnit::kTensorCore, MaskMode::kCausal, false, true, false, false>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
// HEAD_DIM=64, WARP_Q=32, QuantGranularity::kPerThread (3), fuse_v_scale=true, use_inst_buffer=true, Causal
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 32, 64, 64, DataType::kInt8, QuantGranularity::kPerThread, QuantGranularity::kPerThread, float, true, half, ComputeUnit::kTensorCore, MaskMode::kCausal, false, true, false, false>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
// HEAD_DIM=128, WARP_Q=16, QuantGranularity::kPerWarp (2), fuse_v_scale=true, use_inst_buffer=true, Non-causal
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 16, 64, 128, DataType::kInt8, QuantGranularity::kPerWarp, QuantGranularity::kPerWarp, float, true, half, ComputeUnit::kTensorCore, MaskMode::kNone, false, true, false, false>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
// HEAD_DIM=128, WARP_Q=16, QuantGranularity::kPerWarp (2), fuse_v_scale=true, use_inst_buffer=true, Causal
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 16, 64, 128, DataType::kInt8, QuantGranularity::kPerWarp, QuantGranularity::kPerWarp, float, true, half, ComputeUnit::kTensorCore, MaskMode::kCausal, false, true, false, false>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
// HEAD_DIM=64, WARP_Q=32, QuantGranularity::kPerWarp (2), fuse_v_scale=true, use_inst_buffer=true, Non-causal  
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 32, 64, 64, DataType::kInt8, QuantGranularity::kPerWarp, QuantGranularity::kPerWarp, float, true, half, ComputeUnit::kTensorCore, MaskMode::kNone, false, true, false, false>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
// HEAD_DIM=64, WARP_Q=32, QuantGranularity::kPerWarp (2), fuse_v_scale=true, use_inst_buffer=true, Causal
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 32, 64, 64, DataType::kInt8, QuantGranularity::kPerWarp, QuantGranularity::kPerWarp, float, true, half, ComputeUnit::kTensorCore, MaskMode::kCausal, false, true, false, false>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 16, 64, 128, DataType::kInt8, QuantGranularity::kPerWarp, QuantGranularity::kPerWarp, float, true, half, ComputeUnit::kCudaCore, MaskMode::kNone, false, true, false, false>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 16, 64, 128, DataType::kInt8, QuantGranularity::kPerWarp, QuantGranularity::kPerWarp, float, true, half, ComputeUnit::kCudaCore, MaskMode::kCausal, false, true, false, false>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 16, 64, 128, DataType::kInt8, QuantGranularity::kPerBlock, QuantGranularity::kPerBlock, float, true, half, ComputeUnit::kCudaCore, MaskMode::kNone, false, true, false, false>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 16, 64, 128, DataType::kInt8, QuantGranularity::kPerBlock, QuantGranularity::kPerBlock, float, true, half, ComputeUnit::kCudaCore, MaskMode::kCausal, false, true, false, false>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 32, 64, 128, DataType::kInt8, QuantGranularity::kPerWarp, QuantGranularity::kPerWarp, float, true, half, ComputeUnit::kCudaCore, MaskMode::kNone, false, true, false, false>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
...
template __global__ void qk_int_sv_f8_attn_kernel<128, 64, 32, 64, 128, DataType::kInt8, QuantGranularity::kPerWarp, QuantGranularity::kPerWarp, float, true, half, ComputeUnit::kCudaCore, MaskMode::kNone, false, true, false, true>(
    int8_t *__restrict__ Q, int8_t *__restrict__ K, int8_t *__restrict__ V, half *__restrict__ O, float *__restrict__ Lse,
    float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale, float *__restrict__ V_mean,
    const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
    const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q, 
